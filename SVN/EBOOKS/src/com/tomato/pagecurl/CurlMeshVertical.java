...
의 방향에 대한 위치와 한 컬
  ...
  하지만 무슨 일이의 일부 가비지 컬렉션을 방지합니다.*/
	...
	public CurlMeshVertical(int maxCurlSplits) {
...
같은 X 좌표.*/
		...
		for (int i = 0; i < 4; ++i) {
			...
			v.set(mRectangle[i]);
			v.translate(-curlPos.x, -curlPos.y);//양탄자효과는 이걸로 제어해보자 옆페이지인가 현재페이지인가로결정
			//v.rotateZ(-curlAngle);/* 핵심 발견 가로일떼 이것이 어느쪽으로 넘겨주는지의 효과를 결정한다*/
			v.rotateZ(curlAngle);
			int j = 0;
...
//			Vertex v3 = mRotatedVertices.get(2);
			Vertex v2 = mRotatedVertices.get(2);//v2,v3을 바꾸면 역삼각형처럼 넘어
...
//					* (v0.mPosX - v3.mPosX));
		...
		mVerticesCountFront = mVerticesCountBack = 0;//숫자 다른걸 넣으니 다른그림자가 삽입
		...
		// 이 코드 한 하루를 다시 방문합니다. 최적화를위한 공간이 여기에있다.
		mScanLines.clear();
		if (mMaxCurlSplits > 0) {
			mScanLines.add((double)0);
		}
		for (int i = 1; i < mMaxCurlSplits; ++i) {
			mScanLines.add((-curlLength * i) / (mMaxCurlSplits - 1));//넘어가는 효과에서 설정
		}
		...
		/*	mRotatedVertices는 추가, X 좌표에 관한 명령이므로
 			이 스캔 라인은 정점을 따기 스캔 영역을 생성
 			완전히 회전. 하나는 '무한 때까지'라고 말할 수 있습니다.*/
		...
		/* 바로 대부분의 정점에서 시작합니다. 
		        거의 첫번째 스캔 영역과 동일한는 '무한'에서 시작됩니다. */
		...
		for (int i = 0; i < mScanLines.size(); ++i) {
			...
			for (int j = 0; j < mRotatedVertices.size(); ++j) {
				...
				/*	테스트 경우이 검사 지역 정점에 자리잡고 있습니다.
 					TODO : 솔직히 말하기는 평등 확인이 양쪽에 추가했는지
 					 기억이 안나요. 그것을 추측하는 것은 어떻게든 사건과 
 					 관련이 어디에 있었 반경 = 현재의 구현에 따라, 어쨌든
 					  훨씬 더 효과적으로 처리할 수, 0F.*/
				if (v.mPosX >= scanXmin && v.mPosX <= scanXmax) {
					...
					// intersection.
					/* 이것은 삼각 이유로 전적으로 이루어집니다.
					 * 회전된 사각형 감안할 때 그것은 교차하는 데 최대 두 정점 있습니다.*/
					...
					/*의미에서 하나는 우리가 교차 라인의 끝에 위치하고, 
					 * 두 항상 정점을 추가하고 말할 수 있습니다. 그리고 
					 * 삼각 측량을 위해 그들이 Y 좌표를 기반으로 추가됩니다 제대로 작동합니다. 
					 * 그리고이 경우에는 - 다른 우리를 위해 그것을하고있다.*/
					if (intersections.size() == 1//숫자를 바꾸면 접는 부분에 삼각형이 생김
							&& intersections.get(0).mPosY > v.mPosY) {
						// In case intersecting vertex is higher add it first.
						/*버텍스를 교차 하실 경우 먼저 그것을 추가 높다.*/
						mOutputVertices.addAll(intersections);
						mOutputVertices.add(n);
					} else if (intersections.size() <= 1) {
						// Otherwise add original vertex first.
						/* 그렇지 않으면 원래의 버텍스를 먼저 추가합니다*/
						mOutputVertices.add(n);
						mOutputVertices.addAll(intersections);
					} else {
						...
						// everything.
						/*버텍스 교차 1 이상이되지 않을 것입니다. 그것이 대체로서 무슨 일이 생기면 
						 *그러나 단순히 모든 건너 뛰십시오.*/
						mTempVertices.add(n);
			...
			/*/ / 우리는 0 또는 2 정점을 것으로 기대합니다. 
			 * 	드문 경우에 단 하나지만, 일반적으로이 사각형을 교차 스캔 라인이 교차 정점이 있어야 주어진 있어요.
			 */
			if (intersections.size() == 2) {
				// There were two intersections, add them based on y
				// -coordinate, higher first, lower last.
				/*2 교차로 낮은 마지막으로, 높은 우선, Y 좌표를 기반으로 그들을 추가있었습니다.*/
				Vertex v1 = intersections.get(0);
				Vertex v2 = intersections.get(1);
				if (v1.mPosY < v2.mPosY) {
					mOutputVertices.add(v2);//페이지를 넘기는순간 이상2와 1을 바꾸면 갈라진채로 넘어감
				...
				// anything with them.
				/*3 +의 정점이있다면  정확하게 스캔 라인 같은게에서 
				 * 원본 정점이있는 경우에 일어나는 매우 잘못 됐어요. 
				 * 어떤 사상 이유 것은 나중에 사용하기 위해 임시 정점에 정점을 반환합니다. 
				 * 전자의 경우에는 그것은 이미 이전에 한번 후자의 경우에 그들과 함께 아무것도하지 않도록하는 것이 좋습니다, 
				 * ​​mRotatedVertices 통해 iterating 처리되었습니다.
				 * 아마 이 ==문이 들어간 세군대가 접점의 경우를 나타내고있는듯
				 * */
				mTempVertices.addAll(intersections);
			}
			...
			while (mOutputVertices.size() > 0) {
				...
				// Untouched vertices.
				if (i == 0) {
				//	v.mAlpha = mFlipTexture ? BACKFACE_ALPHA : FRONTFACE_ALPHA;//넘기는순간 투명도
					mVerticesCountFront++;
				}
				// 'Completely' rotated vertices.
					...
					v.mPosZ = 2 * radius;
					v.mPenumbraX = v.mPenumbraX;//변화를 주어도 이상이 없음

				//	v.mAlpha = mFlipTexture ? FRONTFACE_ALPHA : BACKFACE_ALPHA;//넘기는순간 투명도
					mVerticesCountBack++;
				}
				// Vertex lies within 'curl'.
					...
					/* 이 경우 - 다른 조항에서 그러는 건 아니하더라도, 여기 v.mPosX은 [0, - curlLength] 사이입니다.
					 * 그리고 우리는 반 실린더 주위 ocalculations 수 있습니다.*/
					...
					v.mColor = .1f + .9f * Math.sqrt(Math.sin(rotY) + 1);//변화를 주어도 이상이 없음

					if (v.mPosZ >= radius) {
				//		v.mAlpha = mFlipTexture ? FRONTFACE_ALPHA //넘기는순간 투명도
					//			: BACKFACE_ALPHA;
						mVerticesCountBack++;
					} else {
					//	v.mAlpha = mFlipTexture ? BACKFACE_ALPHA	//넘기는 순간 투명도
						//		: FRONTFACE_ALPHA;
						mVerticesCountFront++;
					}
				}
				...
				if (DRAW_SHADOW && v.mPosZ > 0 && v.mPosZ <= radius) {//주석처리를 하여도 이상없음
					ShadowVertex sv = mTempShadowVertices.remove(0);
					sv.mPosX = v.mPosX;
					sv.mPosY = v.mPosY;
					sv.mPosZ = v.mPosZ;
					sv.mPenumbraX = (v.mPosZ / 2) * -curlDir.x;//x,y를 바꾸어도 별다른 변화를 못찾아냄
		...
		mVertices.position(0);//0 대신 다른값을 넣으면 넘어가는 방식이 잼있음
		mColors.position(0);
		if (DRAW_TEXTURE) {
			mTexCoords.position(0);
		}
...
/*ㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁㅁ*/
	 ...
	 */
	/*새로운 하나의 생성을 강제로 재설정 할당 텍스처 ID입니다. 
	 * 이 메서드를 호출하면 당신은 대부분 그것이 잃어버린 것에 너무 비트맵을 설정 싶어요. 
	 * 이 방법은 한 번만 예 : GL 컨텍스트가이 방법으로 다시 만들어집니다 
	 * 호출되어야하는 것은 이전의 텍스처 ID를 공개하지 않고 유일하게 새로운 한 다음 렌더링을 요청합니다.
	 */
...
//			int h = bitmap.getWidth();
	...
	public synchronized void setFlipTexture(boolean flipTexture) {//이녀석이 뒷면의 상태를 결정하는듯?????
...
//		mRectangle[2].mPosY = r.bottom;
	...
	private void addVertex(Vertex vertex) {
		...
		if (DRAW_TEXTURE) {
			mTexCoords.put((float) vertex.mTexX);//그림을 붙여넣을때 효과
			mTexCoords.put((float) vertex.mTexY);//그림을 붙여넣을때 효과
		}
	}
...
//			Vertex v2 = vertices.get(lineIndices[j][0]);
			...
			if (v1.mPosX > scanX && v2.mPosX < scanX) {
				...
				if (DRAW_TEXTURE) {
					n.mTexX += (v1.mTexX - v2.mTexX) * c;//손대면 안됨 위험함
					n.mTexY += (v1.mTexY - v2.mTexY) * c;
				}
				if (DRAW_SHADOW) {
					n.mPenumbraX += (v1.mPenumbraX - v2.mPenumbraX) * c;//손대면 안됨 위험함
	...
	private class Vertex {
		...
		/*double x,y를 만지면서 느낀건 좌표값만 원하는대로 바꾸면 컨트롤 가능한게 아닐까라는 추론까지감*/
			...
			double x = mPosX * -sin + mPosY * -cos ;
			double y = mPosX * -cos + mPosY * sin;
			//double x = mPosX * -sin + mPosY * -cos; //드디어 위로 넘기기 완성
			//double y = mPosX * -cos + mPosY * sin;
			//double x = mPosX * cos + mPosY * sin; //가로일때
			//double y = mPosX * -sin + mPosY * cos;//가로일때
	
			mPosX = x;
			mPosY = y;
			double px = mPenumbraX * sin + mPenumbraY * cos;//정확하진 않지만 위로 넘기는구문
			double py = mPenumbraX * cos + mPenumbraY * -sin;
			//double px = mPenumbraX * cos + mPenumbraY * sin;//가로로 넘기는 구문
			//double py = mPenumbraX * -sin + mPenumbraY * cos;//가로일때
          /*반그늘, 반그림자*/
			mPenumbraX = px;
			mPenumbraY = py;
		}
